...
class VirtualMachine(ASObject):
    ...
    def call_ClassInstanceInit(self, index_or_name: Union[ABCMethodIndex, str], *args) -> Any:
        ...
        if self.cbOnInsExe is not None:
          ...
          self.cbOnInsExe.MakeExtraObservation(f'(v.p) FIXIT TODO: temporarily add Class Instance names to local empty namespace in ScopeStack')
    ...
    def call_ClassClassInit(self, index_or_name: Union[ABCMethodIndex, str], *args) -> Any:
        ...
        if self.cbOnInsExe is not None:
          ...
          self.cbOnInsExe.MakeExtraObservation(f'(v.p)environment={BM.DumpVar(environment)}')
          self.cbOnInsExe.MakeExtraObservation(f'(v.p)env.regs={BM.DumpVar(environment.registers)}')
          self.cbOnInsExe.MakeExtraObservation(f'(v.p) FIXIT TODO: temporarily add Class Init names to local empty namespace in ScopeStack')
          self.cbOnInsExe.MakeExtraObservation(f'(v.p)env.SS={BM.DumpVar(environment.scope_stack)}')
          self.cbOnInsExe.MakeExtraObservation(f'(v.p)env.OS={BM.DumpVar(environment.operand_stack)}')
        res =self.execute_code(method_body.code, environment)
        return res
    ...
    def call_static(self, index_or_name: Union[ABCMethodIndex, str], *args) -> Any:
        ...
        if self.cbOnInsExe is not None:
          ...
          self.cbOnInsExe.MakeExtraObservation(f'(v.p)environment={BM.DumpVar(environment)}')
          self.cbOnInsExe.MakeExtraObservation(f'(v.p)env.regs={BM.DumpVar(environment.registers)}')
          self.cbOnInsExe.MakeExtraObservation(f'(v.p) FIXIT TODO: temporarily add some global or static names to local empty namespace in ScopeStack')
          self.cbOnInsExe.MakeExtraObservation(f'(v.p)env.SS={BM.DumpVar(environment.scope_stack)}')
          self.cbOnInsExe.MakeExtraObservation(f'(v.p)env.OS={BM.DumpVar(environment.operand_stack)}')
        res = self.execute_code(method_body.code, environment)
        return res
